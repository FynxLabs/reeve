...
reeve had two disjoint notification paths. Drift detection had a clean sink
abstraction (`internal/drift/sinks`: interface + five adapters + factory),
...
along with two concrete violations in the drift path: the `github_issue` sink
and the sinks factory imported `go-github` directly, and the factory
statically imported every sink.

The split also carried real defects: sinks delivered serially with no
...
- Lift the sink framework into a shared `internal/notify` package: `Sink`
  ...
  `type:` string. Concrete sinks live in `internal/notify/sinks/*` and
  register in `init()`; `internal/notify/all` blank-imports the default set
  so builds can slice.
- Migrate the five drift sinks (slack, webhook, pagerduty, github_issue,
  ...
  becomes the slack sink's PR-event handling.
