...
later parts. It is the index of the run: a reader MUST be able to see every
stack, and which part holds its detail, from the first comment.

Every part MUST state its ordinal and the total, and the first part MUST link to
the others.
...
Stacks past the cap MUST be dropped from the comments, stated in the last part
with a count, and written to the run log.
